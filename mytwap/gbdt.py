...
def modifyData(batch_X, batch_y, batch_flag):
    ...
    for i in range(batch_seq_X.shape[0]):
        ...
        if ((np.isnan(batch_seq_X[i, :]).any() == True) | (np.isnan(batch_seq_y[i]).any() == True) | (
                batch_seq_flag[i] == 0)):
            na_idx[i] = 1
...
def getDataAssumble(prepared_data):
    ...
    data_list = Parallel(n_jobs=PREPARE_JOBS, verbose=0)(
        delayed(modifyData)(item[0], item[1], item[2]) for item in prepared_data)
    batch_seq_X = []
    batch_seq_y = []
    hasData = False
...
def mytrain(stocks, startDate, endDate, predictDate, BATCH_SIZE,ROUND):
    ...
    while batch_start != max_batch:
        ...
        prepared_data = Parallel(n_jobs=PREPARE_JOBS, verbose=0)(
            delayed(get_tick_data)(o['code'], o['date'], database, All_COLUMNS) for o in train_list_now)
        ...
        model_save_path = os.path.join(LOCALDATAPATH, 'lightgbmModel2', str(predictDate) +'_'+ str(current_loop)+'.txt')
        gbm.save_model(model_save_path)
        del prepared_data, inputs, targets
        if current_loop >= ROUND:
            break
        current_loop += 1
    return gbm
...
def mypredict(testInputs,predictDate,ROUND):
    ...
    for i in range(ROUND):
        model_save_path = os.path.join(LOCALDATAPATH, 'lightgbmModel2',str(predictDate) + '_' + str(i+1) + '.txt')
...
# stocks=['600000.SH']
step=50
for i in range(trainNum, len(days) - step-1, step):
    trainStart = days[i - trainNum]
    trainEnd = days[i - 1]
    today = days[i]
    mygbm = mytrain(stocks, trainStart, trainEnd, today, BATCH_SIZE,ROUND)
    ...
    prepared_data = Parallel(n_jobs=PREPARE_JOBS, verbose=0)(
        delayed(get_tick_data)(o['code'], o['date'], database, All_COLUMNS) for o in [test_list[z] for z in batch_idx])
    ...
    predict=mypredict(testInputs,today,ROUND)
